...
class List_Table extends \WP_List_Table {
	...
	function no_items() {
		...
		<div class="stream-list-table-no-items">
			<p><?php esc_html_e( 'No se ha hecho ninguna acción en SIGOES.', 'stream' ) ?></p>
		</div>
		<?php
	}
...
/*funcion column_default formatea los datos del grip de bitacora en wordpress*/
		...
		switch ( $column_name ) {
			case 'date' :
				...
				$date_string = sprintf(
					...
					get_date_from_gmt( $created, 'd-m-Y' )
				);
				$out  = $this->column_link( $date_string, 'date', get_date_from_gmt( $created, 'd-m-Y' ) );
				$out .= '<br />';
			...
			case 'summary' :
				...
				$view_all_text = $object_title ? sprintf( esc_html__( 'Ver la actividad de  "%s"', 'stream' ), esc_attr( $object_title ) ) : esc_html__( 'Ver la actividad realizada', 'stream' );
	...
	function filters_form() {
		...
		$filters_string .= sprintf( '<input type="submit" id="record-query-submit" class="button-primary" value="%s" />', __( 'Filtro', 'stream' ) );
	...
	function filter_select( $name, $title, $items, $ajax = false ) {
		if ( $ajax ) {
			...
			$out = sprintf(
				...
				esc_attr( $name ),
				sprintf( esc_attr__( 'Filtrar por: %s', 'stream' ), $title ),
				implode( '', $options )
			);
		}
...
/*funcion filter_date para capturar y formatear los filtros de fecha*/
	function filter_date( $items ) {
		...
		<div class="date-interval">
			...
			<select class="field-predefined hide-if-no-js" name="date_predefined" data-placeholder="<?php esc_attr_e( 'Todas las fechas', 'stream' ) ?>">
				<option></option>
				<option value="custom" <?php selected( 'custom' === $date_predefined ); ?>><?php esc_attr_e( 'Fecha Personalizada', 'stream' ) ?></option>
				<?php
				foreach ( $items as $key => $interval ) {
					$end = isset( $interval['end'] ) ? $interval['end']->format( 'd-m-Y' ) : null;

					printf(
						'<option value="%s" data-from="%s" data-to="%s" %s>%s</option>',
						esc_attr( $key ),
						esc_attr( $interval['start']->format( 'd-m-Y' ) ),
						esc_attr( $end ),
			...
			<div class="date-inputs">
				<div class="box">
					...
					<input type="text" name="date_from" class="date-picker field-from" placeholder="<?php esc_attr_e( 'Fecha Inicio', 'stream' ) ?>" value="<?php echo esc_attr( $date_from ) ?>" />
				</div>
				<span class="connector dashicons"></span>

				<div class="box">
					<i class="date-remove dashicons"></i>
					<input type="text" name="date_to" class="date-picker field-to" placeholder="<?php esc_attr_e( 'Fecha Fin', 'stream' ) ?>" value="<?php echo esc_attr( $date_to ) ?>" />
				</div>
			</div>
